...
let q1ce2 = 
    Test.make 
    ~name:"Q1c example 2: fc2 is not a valid ftree"
    ~count:1 unit (fun _ -> Q1.validFTree Q1.fc2 = false)
...
let q1de3 = 
    let fs' = Q1.Dir ("root", [ Q1.Dir ("emptb1", [])
                              ; Q1.Dir ("desktop", [ Q1.Dir ("emptb2", [])
                                                   ; Q1.Dir ("empty3", [])
                                             ])
                           ]) 
    in Test.make 
...
(* error code is the numer of failed/errored tests *)
